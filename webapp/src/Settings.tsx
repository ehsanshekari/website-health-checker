...
export default function Settings() {
  ...
  const [newInterval, setNewInterval] = useState<string>('1');
  ...
  async function fetchInterval() {
    try {
      ...
      setNewInterval(String(data.intervalMinutes));
    } catch (err) {
  ...
  async function updateInterval() {
    ...
    try {
      ...
      if (value < 1 || value > 1440) {
        setMessage({ type: 'error', text: 'Interval must be between 1 and 1440 minutes (24 hours)' });
        return;
      }
    ...
    <div style={{ fontFamily: 'system-ui, Arial, sans-serif', padding: 24, maxWidth: 1000 }}>
      ...
      <section style={{ marginBottom: 40 }}>
        ...
        <p style={{ marginTop: 0, marginBottom: 12, color: '#475569' }}>
          Current interval: <strong>{intervalMinutes} minute{intervalMinutes !== 1 ? 's' : ''}</strong>
        </p>
        ...
        <p style={{ fontSize: 14, color: '#64748b', marginTop: 8 }}>
          Min: 1 minute, Max: 1440 minutes (24 hours)
        </p>
      </section>
